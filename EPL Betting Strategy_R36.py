"""
Premier League Betting Strategy - COMPLETE ROLLING MODIFICATION MODEL (FINAL FIXED)
Fixed: File saving path, run until data ends (not just 20 rounds)
"""
...
SAVE_PATH = SCRIPT_DIR if SCRIPT_DIR else os.getcwd()
...
def load_and_preprocess_data(file_path):
    """Load and preprocess historical data, identify upcoming matches"""
    log("="*80)
    log("STEP 1: LOADING HISTORICAL DATA")
    log("="*80)
    ...
    # For upcoming matches, set default time weight
...
def plot_random_forest_architecture():
    ...
    filepath = os.path.join(SAVE_PATH, 'A1_random_forest_architecture.png')
...
def plot_gradient_boosting_architecture():
    ...
    filepath = os.path.join(SAVE_PATH, 'A2_gradient_boosting_architecture.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_xgboost_architecture():
    ...
    filepath = os.path.join(SAVE_PATH, 'A3_xgboost_architecture.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
# ============================================================================
# 5. ROLLING MODIFICATION SIMULATION (RUN UNTIL DATA ENDS)
# ============================================================================
...
def rolling_modification_simulation(df, team_encoder, all_teams, feature_cols, best_model_info,
    ...
    while pos + round_size <= total_matches:
        ...
        profit_history.append({'round': round_num, 'round_profit': metrics['total_profit'], 
                               'cumulative_profit': cum_profit, 'roi': metrics['roi'], 'accuracy': 0.5})
...
def calculate_capital_metrics(profit_history):
    if not profit_history:
        return {'max_capital': 0, 'payback_round': None, 'payback_days': None, 'irr': None}
    ...
    irr = None
    if max_capital > 0 and payback_round:
        total_profit = cumulative[payback_round - 1]
        if total_profit > 0:
            years = payback_round * 10 / 365
            irr = ((total_profit / max_capital) ** (1 / years) - 1) * 100
    
    return {'max_capital': max_capital, 'payback_round': payback_round, 
            'payback_days': payback_round * 10 if payback_round else None, 'irr': irr}

# ============================================================================
# 7. VISUALIZATIONS (SAVE TO PATH)
# ============================================================================
...
def plot_model_comparison(results):
    ...
    filepath = os.path.join(SAVE_PATH, '1_model_comparison.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_hyperparameter_tuning(tuning_results):
    ...
    filepath = os.path.join(SAVE_PATH, '2_hyperparameter_tuning.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_profit_over_rounds(profit_history):
    ...
    filepath = os.path.join(SAVE_PATH, '3_profit_over_rounds.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")

def plot_roi_vs_accuracy(profit_history):
    if not profit_history: return
    fig, ax = plt.subplots(figsize=(10, 8))
    rois = [p['roi'] for p in profit_history]
    accuracies = [p.get('accuracy', 0.5) for p in profit_history]
    rounds = [p['round'] for p in profit_history]
    colors = ['#2ecc71' if r>0 else '#e74c3c' for r in rois]
    ax.scatter(accuracies, rois, c=colors, s=100, alpha=0.7, edgecolor='black')
    for i, r in enumerate(rounds):
        ax.annotate(str(r), (accuracies[i], rois[i]), xytext=(5,5), textcoords='offset points', fontsize=9)
    ax.axhline(y=0, color='black'); ax.axvline(x=0.5, color='gray', linestyle='--', alpha=0.7)
    ax.set_xlabel('Model Accuracy'); ax.set_ylabel('ROI (%)'); ax.set_title('ROI vs Accuracy Correlation', fontweight='bold')
    ax.grid(True, alpha=0.3)
    filepath = os.path.join(SAVE_PATH, '4_roi_vs_accuracy.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_feature_evolution(feature_importance_history, feature_cols, top_n=10):
    ...
    filepath = os.path.join(SAVE_PATH, '5_feature_evolution.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_final_features(feature_importance_history):
    ...
    filepath = os.path.join(SAVE_PATH, '6_final_features.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_capital_metrics(capital_metrics):
    ...
    Maximum Capital Employed:     ¥{capital_metrics['max_capital']:.2f}
    Payback Period:               {capital_metrics['payback_round']} rounds ({capital_metrics['payback_days']} days)
    IRR (Annualized):             {capital_metrics['irr']:.2f}% if capital_metrics['irr'] else 'N/A'
    ...
    filepath = os.path.join(SAVE_PATH, '7_capital_metrics.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def plot_betting_summary(selected_bets):
    ...
    ax1.set_title('Round 35 - Stake Allocation (¥100)', fontweight='bold')
    ax2 = axes[1]
    ...
    ax2.set_title('Round 35 - Expected Value by Bet', fontweight='bold')
    ...
    ax2.grid(True, alpha=0.3, axis='y')
    filepath = os.path.join(SAVE_PATH, '8_round35_summary.png')
    plt.savefig(filepath, dpi=150, bbox_inches='tight')
    plt.close()
    log(f"✓ Saved: {filepath}")
...
def main():
    ...
    log(f"文件保存路径: {SAVE_PATH}")
    log("="*80)
    
    file_path = r'C:\Users\Administrator\Desktop\CourseS2\7780\Premier League_data\season22-25.csv'
    ...
    try:
        ...
        # Rolling simulation (run until data ends)
        ...
        # Capital metrics
        capital_metrics = calculate_capital_metrics(profit_history)
        ...
        plot_roi_vs_accuracy(profit_history)
        ...
        log("FINAL SUMMARY")
        log("="*80)
        total_profit = profit_history[-1]['cumulative_profit'] if profit_history else 0
        log(f"  Total Rounds: {len(round_metrics)}")
        log(f"  Cumulative Profit: ¥{total_profit:.2f}")
        log(f"  Max Capital Required: ¥{capital_metrics['max_capital']:.2f}")
        log(f"  Payback Period: {capital_metrics['payback_round']} rounds ({capital_metrics['payback_days']} days)")
        log(f"  IRR: {capital_metrics['irr']:.2f}%" if capital_metrics['irr'] else "  IRR: N/A")
        ...
        log(f"  {os.path.join(SAVE_PATH, '4_roi_vs_accuracy.png')}")
        log(f"  {os.path.join(SAVE_PATH, '5_feature_evolution.png')}")
        log(f"  {os.path.join(SAVE_PATH, '6_final_features.png')}")
        log(f"  {os.path.join(SAVE_PATH, '7_capital_metrics.png')}")
        log(f"  {os.path.join(SAVE_PATH, '8_round35_summary.png')}")
